...
func TestRoutingPickerGroupPick(t *testing.T) {
	tests := []struct {
		...
		pickers map[internal.LocalityID]*routingSubBalancerState
		...
		},
		{
			...
			routes: []routingPickerRoute{
				{m: newPathPrefixMatcher("/a/"), id: "action-0"},
			},
			pickers: map[internal.LocalityID]*routingSubBalancerState{
		...
		},
		{
			...
			routes: []routingPickerRoute{
				{m: newPathPrefixMatcher("/a/"), id: "action-0"},
				{m: newPathPrefixMatcher("/z/"), id: "action-1"},
			},
			pickers: map[internal.LocalityID]*routingSubBalancerState{
				makeLocalityFromName("action-0"): {state: balancer.State{
					ConnectivityState: connectivity.Ready,
		...
		},
		{
			...
			routes: []routingPickerRoute{
				{m: newPathExactMatcher("/a/b"), id: "action-0"},
				{m: newPathPrefixMatcher("/a/"), id: "action-1"},
			},
			pickers: map[internal.LocalityID]*routingSubBalancerState{
				makeLocalityFromName("action-0"): {state: balancer.State{
					ConnectivityState: connectivity.Ready,
		...
		},
		{
			...
			routes: []routingPickerRoute{
				{m: newPathPrefixMatcher("/a/"), id: "action-0"},
				{m: newPathExactMatcher("/a/b"), id: "action-1"},
			},
			pickers: map[internal.LocalityID]*routingSubBalancerState{
				makeLocalityFromName("action-0"): {state: balancer.State{
					ConnectivityState: connectivity.Ready,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			pg := newRoutingPickerGroup(tt.routes, tt.pickers)

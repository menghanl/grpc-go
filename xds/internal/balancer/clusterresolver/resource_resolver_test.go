...
func (s) TestResourceResolverOneEDSResource(t *testing.T) {
	for _, test := range []struct {
		...
		want                 []balancerconfigbuilder.PriorityConfig
	}{
		{name: "watch EDS",
		...
		t.Run(test.name, func(t *testing.T) {
			...
			rr := newResourceResolver(fakeClient, nil)
			rr.updateMechanisms([]balancerconfigbuilder.DiscoveryMechanism{{
				Type:           balancerconfigbuilder.DiscoveryMechanismTypeEDS,
...
func (s) TestResourceResolverChangeEDSName(t *testing.T) {
	...
	select {
	case u := <-rr.updateChannel:
		if diff := cmp.Diff(u.p, []balancerconfigbuilder.PriorityConfig{{
			Mechanism: balancerconfigbuilder.DiscoveryMechanism{
				Type:           balancerconfigbuilder.DiscoveryMechanismTypeEDS,
				Cluster:        testClusterName,
				EDSServiceName: testEDSServcie,
...
func (s) TestResourceResolverNoChangeNoUpdate(t *testing.T) {
	...
	if edsNameCanceled2 != gotEDSName2 {
		t.Fatalf("xdsClient.CancelEDS called for %v, want: %v", edsNameCanceled2, gotEDSName2)
	}
}

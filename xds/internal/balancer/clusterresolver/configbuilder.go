...
func buildClusterImplConfigForEDS(parentPriority int, edsResp xdsclient.EndpointsUpdate, mechanism DiscoveryMechanism) ([]string, map[string]*clusterimpl.LBConfig, []resolver.Address) {
	var (
		retNames   []string
		retAddrs   []resolver.Address
	...
	for _, priorityName := range priorityChildNames {
		...
		wtConfig, addrs := localitiesToWeightedTarget(priorityLocalities, pName, childPolicy, lrsServer, cluster, edsService)

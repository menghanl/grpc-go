...
func (dr *dnsResolver) NewAddress(addresses []resolver.Address) {
	dr.UpdateState(resolver.State{
		Addresses: addresses,
	})
}

func (dr *dnsResolver) NewServiceConfig(serviceConfig string) {
	// Deprecated, and service config isn't support in cluster_resolver.
}

func (dr *dnsResolver) ParseServiceConfig(serviceConfigJSON string) *serviceconfig.ParseResult {

...
import (
	...
	_ "google.golang.org/grpc/xds/internal/xdsclient/controller/v2" // V2 client registration.
)
...
func (s) TestErrorFromXDSClientUpdate(t *testing.T) {
	...
	connectionErr := resource.NewErrorf(resource.ErrorTypeConnection, "connection error")
	...
	resourceErr := resource.NewErrorf(resource.ErrorTypeResourceNotFound, "clusterResolverBalancer resource not found error")

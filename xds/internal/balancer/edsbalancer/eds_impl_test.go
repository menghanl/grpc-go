...
func (s) TestEDS_EndpointsHealth(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		sc := <-cc.NewSubConnCh
		edsb.handleSubConnStateChange(sc, balancer.SubConnState{ConnectivityState: connectivity.Connecting})
		edsb.handleSubConnStateChange(sc, balancer.SubConnState{ConnectivityState: connectivity.Ready})
...
func (s) TestClose(_ *testing.T) {
	edsb := newEDSBalancerImpl(nil, balancer.BuildOptions{}, nil, nil)
...
func (s) TestEDS_UpdateSubBalancerName(t *testing.T) {
	...
	edsb.handleServiceConfig(&EDSConfig{ChildPolicy: &loadBalancingConfig{Name: balancerName}}, nil)
	...
	edsb.handleServiceConfig(&EDSConfig{ChildPolicy: &loadBalancingConfig{Name: roundrobin.Name}}, nil)
...
func (s) TestEDS_CircuitBreaking(t *testing.T) {
	...
	edsb.handleServiceConfig(&EDSConfig{
...
func (s) TestEDS_ChildPolicyUpdatePickerInline(t *testing.T) {
	...
	edsb := newEDSBalancerImpl(cc, balancer.BuildOptions{}, nil, nil)
	defer edsb.close()
	edsb.enqueueChildBalancerStateUpdate = func(state balancer.State) {
		// For this test, euqueue needs to happen asynchronously (like in the
		// real implementation).
		go cc.UpdateState(state)
	}

	edsb.handleServiceConfig(&EDSConfig{ChildPolicy: &loadBalancingConfig{Name: "test-inline-update-balancer"}}, nil)

	clab1 := testutils.NewClusterLoadAssignmentBuilder(testClusterNames[0], nil)
	clab1.AddLocality(testSubZones[0], 1, 0, testEndpointAddrs[:1], nil)
	edsb.handleEDSResponse(parseEDSRespProtoForTesting(clab1.Build()))

	if err := testErrPickerFromCh(cc.NewPickerCh, errTestInlineStateUpdate); err != nil {
		t.Fatal(err)
	}
}

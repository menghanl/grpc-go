...
func (ew *edsWatcher) updateConfig(config *EDSConfig) {
	...
	cancelEDSWatch := ew.xdsClient.WatchEndpoints(newEDSToWatch, func(update xdsclient.EndpointsUpdate, err error) {
	...
	ew.logger.Infof("Watch started on resource name %v with xds-client %p", newEDSToWatch, ew.xdsClient)
	ew.edsCancel = func() {
		cancelEDSWatch()
		ew.logger.Infof("Watch cancelled on resource name %v with xds-client %p", newEDSToWatch, ew.xdsClient)
	}

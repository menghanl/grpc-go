...
func Test_build(t *testing.T) {
	...
	got, _ := build(xdsclient.EndpointsUpdate{
		...
		&EDSConfig{
			...
			LrsLoadReportingServerName: newString(testLRSServer),
		})

	var prettyGot bytes.Buffer
	if err := json.Indent(&prettyGot, got, ">>> ", "  "); err != nil {
		t.Fatalf("%v", err)
	}
	fmt.Println(prettyGot.String())

	priorityB := balancer.Get(priority.Name)
	gg, err := priorityB.(balancer.ConfigParser).ParseConfig(got)
	if err != nil {
		t.Fatalf("%v", err)
	}
	// Dump(gg)
...
func TestBuildPriorityConfig(t *testing.T) {
	...
	gotConfig, gotAddrs := buildPriorityConfig(
	...
	wantConfig := &priority.LBConfig{
			...
			"priority-0": {
				Config: &internalserviceconfig.BalancerConfig{
					...
					Config: &clusterimpl.LBConfig{
						...
						LRSLoadReportingServerName: newString(testLRSServer),
						MaxConcurrentRequests:      newUint32(testMaxReq),
						DropCategories: []clusterimpl.DropCategory{
							{
								Category:           testDropCategory,
									...
									assertString(testLocalityIDs[0].ToString): {
										...
										ChildPolicy: &internalserviceconfig.BalancerConfig{
											...
											Config: &lrs.LBConfig{
												ClusterName:                testClusterName,
												EdsServiceName:             testEDSServiceName,
												LrsLoadReportingServerName: testLRSServer,
		...
		Priorities: []string{"priority-0", "priority-1"},
	}
	wantAddrs := []resolver.Address{
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[0][0]}, []string{"priority-0", assertString(testLocalityIDs[0].ToString)}),
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[0][1]}, []string{"priority-0", assertString(testLocalityIDs[0].ToString)}),
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[1][0]}, []string{"priority-0", assertString(testLocalityIDs[1].ToString)}),
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[1][1]}, []string{"priority-0", assertString(testLocalityIDs[1].ToString)}),
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[2][0]}, []string{"priority-1", assertString(testLocalityIDs[2].ToString)}),
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[2][1]}, []string{"priority-1", assertString(testLocalityIDs[2].ToString)}),
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[3][0]}, []string{"priority-1", assertString(testLocalityIDs[3].ToString)}),
		hierarchy.Set(resolver.Address{Addr: testAddressStrs[3][1]}, []string{"priority-1", assertString(testLocalityIDs[3].ToString)}),
	}
...
func TestGroupLocalitiesByPriority(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantPriorities: []string{"priority-0"},
			wantLocalities: map[string][]xdsclient.Locality{
				"priority-0": {testLocalitiesP0[0]},
			},
		},
		{
			...
			wantPriorities: []string{"priority-0"},
			wantLocalities: map[string][]xdsclient.Locality{
				"priority-0": {testLocalitiesP0[0], testLocalitiesP0[1]},
			},
		},
		{
			name:           "1 locality in each",
			localities:     []xdsclient.Locality{testLocalitiesP0[0], testLocalitiesP1[0]},
			wantPriorities: []string{"priority-0", "priority-1"},
			wantLocalities: map[string][]xdsclient.Locality{
				"priority-0": {testLocalitiesP0[0]},
				"priority-1": {testLocalitiesP1[0]},
			},
		},
		{
			...
			wantLocalities: map[string][]xdsclient.Locality{
				"priority-0": {testLocalitiesP0[0], testLocalitiesP0[1]},
				"priority-1": {testLocalitiesP1[0], testLocalitiesP1[1]},
			},
		},
		{
			...
			wantPriorities: []string{"priority-0", "priority-1"},
			wantLocalities: map[string][]xdsclient.Locality{
				"priority-0": {testLocalitiesP0[1], testLocalitiesP0[0]},
				"priority-1": {testLocalitiesP1[1], testLocalitiesP1[0]},
			},
		},
	}
...
func TestLocalitiesToWeightedTarget(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			priorityName: "test-priority",
			childPolicy:  weightedroundrobin.Name,
			// lrsServer is nil, so LRS policy will not be used.
			wantConfig: &weightedtarget.LBConfig{
				Targets: map[string]weightedtarget.Target{
					assertString(internal.LocalityID{Zone: "test-zone-1"}.ToString): {

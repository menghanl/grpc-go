...
func (s) TestLDSWatch(t *testing.T) {
	...
	if _, err := apiClient.addWatches[resource.ListenerResource].Receive(ctx); err != nil {
...
func (s) TestLDSWatchAfterCache(t *testing.T) {
	...
	sCtx, sCancel := context.WithTimeout(ctx, defaultTestShortTimeout)
	defer sCancel()
	if n, err := apiClient.addWatches[resource.ListenerResource].Receive(sCtx); err != context.DeadlineExceeded {
...
func (s) TestLDSResourceRemoved(t *testing.T) {
	...
	if u, err := ldsUpdateCh1.Receive(ctx); err != nil || resource.ErrType(u.(resource.ListenerUpdateErrTuple).Err) != resource.ErrorTypeResourceNotFound {
...
func (s) TestListenerWatchPartialValid(t *testing.T) {
	...
	for _, name := range []string{testLDSName, badResourceName} {
		...
		defer func() {
			cancelWatch()
			if _, err := apiClient.removeWatches[resource.ListenerResource].Receive(ctx); err != nil {

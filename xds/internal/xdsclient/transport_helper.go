...
type VersionedClient interface {
	...
	SendRequest(s grpc.ClientStream, resourceNames []string, rType resource.ResourceType, version, nonce, errMsg string) error
	...
	HandleResponse(proto.Message) (resource.ResourceType, string, string, error)
...
func (t *TransportHelper) AddWatch(rType resource.ResourceType, resourceName string) {
...
func (t *TransportHelper) RemoveWatch(rType resource.ResourceType, resourceName string) {
	t.sendCh.Put(&watchAction{
		rType:    rType,
		remove:   true,
		resource: resourceName,
...
func (t *TransportHelper) processWatchInfo(w *watchAction) (target []string, rType resource.ResourceType, ver, nonce string) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *TransportHelper) processAckInfo(ack *ackAction, stream grpc.ClientStream) (target []string, rType resource.ResourceType, version, nonce string, send bool) {

...
type VersionedClient interface {
	...
	SendRequest(s grpc.ClientStream, resourceNames []string, rType resource.Type, version, nonce, errMsg string) error
	...
	// FIXME: DELETE THIS
	HandleResponse(r proto.Message) (resource.Type, string, string, error)
...
func (t *Controller) AddWatch(rType resource.Type, resourceName string) {
...
func (t *Controller) RemoveWatch(rType resource.Type, resourceName string) {
	t.sendCh.Put(&watchAction{
		rType:    rType,
		remove:   true,
		resource: resourceName,
...
func (t *Controller) sendExisting(stream grpc.ClientStream) bool {
	...
	t.versionMap = make(map[resource.Type]string)
	t.nonceMap = make(map[resource.Type]string)
...
func (t *Controller) processWatchInfo(w *watchAction) (target []string, rType resource.Type, ver, nonce string) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *Controller) processAckInfo(ack *ackAction, stream grpc.ClientStream) (target []string, rType resource.Type, version, nonce string, send bool) {
...
func (t *Controller) reportLoad(ctx context.Context, cc *grpc.ClientConn, opts LoadReportingOptions) {
	retries := 0
	for {
		if ctx.Err() != nil {
			return

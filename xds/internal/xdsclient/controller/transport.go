...
import (
	...
	version2 "google.golang.org/grpc/xds/internal/xdsclient/controller/version"
...
func (t *Controller) recv(stream grpc.ClientStream) bool {
	...
	for {
		...
		if e, ok := err.(version2.ErrResourceTypeUnsupported); ok {
...
func (t *Controller) handleResponse(resp proto.Message) (xdsresource.ResourceType, string, string, error) {
	...
	fmt.Printf(" \n\n\n %v, %v, %v, %v, %v \n\n\n", rType, resource, version, nonce, err)
	...
	default:
		return rType, "", "", version2.ErrResourceTypeUnsupported{
...
func (t *Controller) reportLoad(ctx context.Context, cc *grpc.ClientConn, opts version.LoadReportingOptions) {
	retries := 0
	for {
		if ctx.Err() != nil {
			return

...
type route struct {
	prefix  string
	headers [][]string // list of [key, value] pairs
	action  map[string]uint32
}

func makeRouteConfig(ldsName, rdsName string, routes []route) *xdspb.RouteConfiguration {
	var rts []*routepb.Route
	for _, t := range routes {
		var routeAction *routepb.RouteAction
		clusterWeights := t.action
		...
		rts = append(rts,
			&routepb.Route{
				Match: &routepb.RouteMatch{
					PathSpecifier: &routepb.RouteMatch_Prefix{Prefix: t.prefix},
...
func makeCluster(cdsName, edsName string) *xdspb.Cluster {
	return &xdspb.Cluster{
		...
		LrsServer: &corepb.ConfigSource{
			ConfigSourceSpecifier: &corepb.ConfigSource_Self{Self: &corepb.SelfConfigSource{}},
		},
	}
}
...
func makeClusterLoadAssignment(edsName string, localityName, backendAddr string /*this should ba a map[map[]]*/) *xdspb.ClusterLoadAssignment {
	clab0 := testutils.NewClusterLoadAssignmentBuilder(edsName, nil)
...
var (
	...
	goodRouteConfig1 = makeRouteConfig("lds.name", "route.name",
		[]route{
			...
			// {
			// 	prefix: "/grpc.examples.echo.Echo/U",
			// 	action: map[string]uint32{
			// 		"cluster-1.name": 50,
			// 		"cluster-2.name": 50,
			// 	},
...
func main() {
	...
	for {
		...
		req, err := s.XDSRequestChan.Receive(ctxT)
		if err != nil {
